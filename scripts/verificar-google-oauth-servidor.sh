...
CALLBACK_PROXY_URL="$([ "$BACKEND_URL" != "NO CONFIGURADO" ] && echo "${BACKEND_URL}/api/auth/invitado/google/callback-proxy" || echo "https://amva.org.es/api/auth/invitado/google/callback-proxy")"
echo ""
echo "5. Callback URL (web): $CALLBACK_URL"
echo "6. Callback PROXY URL (app móvil): $CALLBACK_PROXY_URL"
echo "   → AMBAS deben estar en Google Cloud Console (Authorized redirect URIs)"
echo ""
echo "========================================"
...
echo "   3. Agrega redirect URIs: $CALLBACK_URL y $CALLBACK_PROXY_URL"
