  ...
  return useQuery({
    ...
    queryFn: () => {
      if (!convencionId || !email) {
        console.log('[useCheckInscripcion] Query deshabilitada - faltan parámetros:', {
          convencionId,
          email,
        })
        return null
      }
      console.log('[useCheckInscripcion] Ejecutando query para verificar inscripción:', {
        convencionId,
        email,
      })
    ...
    onError: (error: any) => {
      ...
      if (error.response?.status === 409) {
        ...
        const errorMessage =
          ...
          error.message ||
          'Este correo electrónico ya está registrado para esta convención'
