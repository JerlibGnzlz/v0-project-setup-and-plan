  ...
  return useMutation({
    ...
    onError: (error: any) => {
      ...
      if (error.response?.status === 409) {
        ...
        const errorMessage = responseData?.error?.message || responseData?.message || error.message || "Ya existe un pastor con este correo electrónico"
      ...
      if (error.response?.status === 400) {
        ...
        if (responseData?.error?.message) {
          ...
          const validationErrors = responseData.error.details.validationErrors
            .map((err: any) => typeof err === 'string' ? err : `${err.field || 'campo'}: ${err.message || err}`)
            .join(', ')
      ...
      const errorMessage = error.response?.data?.error?.message || error.response?.data?.message || error.message || "Error al crear el pastor"
...
export function useUpdatePastor() {
  ...
  return useMutation({
    ...
    onError: (error: any) => {
      ...
      if (error.response?.status === 409) {
        const responseData = error.response.data
        const errorMessage = responseData?.error?.message || responseData?.message || error.message || "Ya existe otro pastor con este correo electrónico"
      ...
      const errorMessage = error.response?.data?.error?.message || error.response?.data?.message || error.message || "Error al actualizar el pastor"
