...
export function PagoWizard({
  ...
  const esInscripcionManual =
    inscripcion.origenRegistro === 'dashboard' || !inscripcion.origenRegistro
  const metodosDisponibles = esInscripcionManual
    ? METODOS_PAGO_MANUAL
    : [...METODOS_PAGO_WEB, ...METODOS_PAGO_MANUAL]
  ...
  const puedeValidar = referenciaEsOpcional
    ? requiereComprobante
      ? comprobanteUrl
      : true // Solo requiere comprobante si aplica
              ...
              {esInscripcionManual && !inscripcion.codigoReferencia && (
                <div className="p-3 bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800 rounded-lg">
                  <p className="text-xs text-blue-700 dark:text-blue-300 flex items-center gap-2">
                    ...
                    <span>
                      Esta inscripción no tiene código de referencia. Puedes crear el pago sin
                      referencia o agregar un número de recibo opcional.
                    </span>
                  </p>
                </div>
              )}
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <div
                            className={cn(
                              'p-2 rounded-lg',
                              isSelected ? `${metodo.colorClasses.iconBg} text-white` : 'bg-muted'
                            )}
                          >
                        ...
                        {isSelected && (
                          <CheckCircle2 className={cn('size-5', metodo.colorClasses.text)} />
                        )}
                      </div>
                    </button>
                  )
                ...
                {puedeValidar && (
                  <div className="p-3 bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800 rounded-lg">
                    <p className="text-xs text-emerald-700 dark:text-emerald-300 flex items-center gap-2">
                      ...
                      <strong>Validación automática:</strong> Este pago se validará automáticamente
                      al guardar
                    </p>
                  </div>
                )}
