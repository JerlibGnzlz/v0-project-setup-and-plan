...
export default function GaleriaPage() {
  ...
  const [editTrimOptions, setEditTrimOptions] = useState<{
    startTime: number
    endTime: number
  } | null>(null)
  ...
  const handleVideoUpload = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const uploadResult = await uploadApi.uploadVideo(videoFile, videoTrimOptions || undefined)
              ...
              {imagenes.length === 0 ? (
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {imagenes.map((imagen: GaleriaImagen) => (
                    <div
                      key={imagen.id}
                      className="relative aspect-[4/3] rounded-lg border bg-muted/20 group overflow-hidden"
                    >
                      <img
          ...
          <Card className="border-rose-200/50 dark:border-rose-500/20 bg-gradient-to-br from-white to-rose-50/30 dark:from-background dark:to-rose-950/20 overflow-hidden">
            ...
            <CardHeader className="flex flex-row items-center justify-between">
              <div>
                ...
                <CardDescription>
                  Sube hasta {MAX_VIDEOS} videos (máx. 2 min cada uno). Puedes recortar videos
                  largos.
                </CardDescription>
              </div>
              <Button
              ...
              {videos.length === 0 ? (
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  {videos.map((video: GaleriaImagen) => (
                    <div
                      key={video.id}
                      className="relative aspect-video rounded-lg border overflow-hidden group bg-black"
                    >
                      <video
                        src={video.imagenUrl}
                      ...
                      <div className="absolute bottom-0 left-0 right-0 p-2 bg-gradient-to-t from-black/80 to-transparent pointer-events-none">
                        <div className="flex justify-between items-center">
                          ...
                          {video.videoStartTime !== undefined &&
                            video.videoEndTime !== undefined && (
                              <Badge
                                variant="outline"
                                className="text-[10px] bg-black/50 text-white border-white/30 ml-2"
                              >
