...
import { CredencialCapellaniaFlipCardCompact } from '@/components/admin/credenciales-capellania/credencial-capellania-flip-card-compact'
...
export default function VisorCredencialesPage() {
  ...
  const [selectedCredencial, setSelectedCredencial] =
    useState<CredencialCapellania | null>(null)
  ...
  const [credencialToDelete, setCredencialToDelete] = useState<CredencialCapellania | null>(null)
  ...
  const { data, isLoading, error } = useCredencialesCapellania(page, limit, filters)
  const deleteMutation = useDeleteCredencialCapellania()
  ...
  const handleCredencialCreated = (credencial: CredencialCapellania) => {
  ...
  const handleEditFrente = (credencial: CredencialCapellania) => {
    setSelectedCredencial(credencial)
    setEditMode('frente')
    setIsDialogOpen(true)
  }

  const handleEditDorso = (credencial: CredencialCapellania) => {
    setSelectedCredencial(credencial)
    setEditMode('dorso')
    setIsDialogOpen(true)
  }

  const handleView = (credencial: CredencialCapellania) => {
    setSelectedCredencial(credencial)
    setViewMode('view')
  }

  const handleDelete = (credencial: CredencialCapellania) => {
  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Credenciales de Capellanía</h1>
          <p className="text-muted-foreground">
          ...
          <CardContent className="pt-6">
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-semibold mb-2">
                No hay credenciales de capellanía
              </h3>
                      ...
                      <tr key={credencial.id} className="border-b">
                        <td className="p-3">
                          <div className="relative" style={{ minHeight: '280px' }}>
                            <CredencialCapellaniaFlipCardCompact credencial={credencial} />
                          </div>
                        </td>
                        <td className="p-3">
                          <div>
                        ...
                        <td className="p-3">
                          <span className="text-sm">{credencial.tipoCapellan}</span>
                        </td>
                        <td className="p-3">
                          <div>
