...
import { CheckCircle2, Calendar as CalendarIcon, Clock, MapPin } from 'lucide-react-native'
...
export function ConventionInscripcionScreen() {
  ...
  const handleStepComplete = async (step: number, data?: any) => {
    ...
    // Si step === 2 y data es una inscripción, actualizar estado inmediatamente
    if (step === 2 && data && data.id) {
      console.log('✅ Inscripción recibida en handleStepComplete:', data)
      setYaInscrito(true)
    ...
    Animated.parallel([
      ...
      if (step === 1 && isAuthenticated) {
        ...
        setInscripcionCompleta(true)
        // Si no recibimos la inscripción en data, intentar recargarla
        if (!data || !data.id) {
            ...
            <>
              {yaInscrito && inscripcionExistente ? (
                ...
                <Step2UnifiedForm
                  ...
                  onComplete={(inscripcion) => handleStepComplete(2, inscripcion)}
                  onBack={handleBack}
                />
              )}
            </>
