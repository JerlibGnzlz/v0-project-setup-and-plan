...
const EXPO_PUBLIC_API_URL = typeof process !== 'undefined' ? process.env?.EXPO_PUBLIC_API_URL : undefined
...
apiClient.interceptors.response.use(
    ...
    async (error: AxiosError) => {
        ...
        if (error.response?.status === 401 && !originalRequest._retry) {
            ...
            const isAuthEndpoint = originalRequest.url?.includes('/auth/pastor/login') ||
